...
def create_client(model_name: str, temperature: float = 0.75) -> Tuple[ChatOpenAI, str]:
    """Create a LangChain chat model and return it with the model name."""
    model_map = {
        "deepseek": (ChatOpenAI, "DEEPSEEK_API_KEY", "https://api.deepseek.com/v1"),
        "claude": (ChatAnthropic, "ANTHROPIC_API_KEY", None),
        "gpt": (ChatOpenAI, "OPENAI_API_KEY", None)
    }
    
    for key, (model_class, api_key_env, base_url) in model_map.items():
        if key in model_name:
            ...
            print(f"Using LangChain with model {model_name}")
